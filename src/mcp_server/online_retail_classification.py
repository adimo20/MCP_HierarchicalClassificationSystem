...
class ClassificationSignature(dspy.Signature):
    ...
    You will be provided with the product's name, price, descriptive details, and the retailer's internal category. 
    ...
    1. Analyze the Input: Evaluate the product data to form an initial hypothesis about its broad category (e.g., Food, Clothing, Electronics).
    ...
    5. Verify: That you have reached SEA level 4, which means that you found a 5 digit code.
    6. Avoid Over-specification: If your search leads to a highly specific code (e.g., "Men's T-Shirts"), but the input data is vague (e.g., just "T-Shirt"), you MUST use `get_parent` to step back up to the safest, broader parent category. Never guess missing demographics or materials.
    
    Output the final, most appropriate SEA code string.
    """
    product_name: str = dspy.InputField(desc="Product to classify.")
    price: str = dspy.InputField(desc="Price of the product to classify.")
    details: str = dspy.InputField(desc="Details provided by the only retailer.")
    retailer_category: str = dspy.InputField(desc = "Product category the retailer lists this product as.")
    
    sea: str = dspy.OutputField(desc="The most proper fitting Category of the Systematik der Einnahmen und Ausgaben Privater Haushalte (SEA) you find in the classification system.")
...
if __name__ == "__main__":
    ...
    answer = classification_agent.agent(
        product_name=  "Damen T-Shirt, Weiß, S 36/38",
        price= "4,99 €",
        details=rf"ProduktdetailsJe StückGrößen: S (36/38) - L (44/46)Regular Fit100 % BaumwollePflegehinweise: 30 °C SchonwaschgangVerschiedene Farben:ElfenbeinBlauRosaKleidung kann gegen Vorlage des Kassenbons innerhalb von 3 Monaten ab Kaufdatum umgetauscht werden.",
        retailer_category="Kleidung" 
    )
